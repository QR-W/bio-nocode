...
export default function StatsCardsWidget({ records, config, props }: WidgetProps) {
    ...
    const items = [
        showTotal && { title: '总记录数', value: records.length, suffix: '条', color: valueColor ?? '#4F46E5', isText: false },
        ...
        showLastDate && {
            title: '最近录入',
            value: records[0] ? new Date(records[0].createdAt).toLocaleDateString('zh-CN') : '暂无',
            suffix: '',
